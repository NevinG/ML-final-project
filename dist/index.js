    ...
    for (let i = 0; i < inputPixels.length; i++) {
        for (let j = 0; j < layers[0].length; j++) {
            if (inputPixels[i].value > 0) {
                renderConnection({ perceptron: null, x: inputPixels[i].x, y: inputPixels[i].y, r: 0 }, layers[0][j], layers[0][j].perceptron.weights[i] * 4 //so you can see them better
                );
            }
        }
    }
...
function renderConnection(perceptronA, perceptronB, weight) {
    ctx.beginPath();
